...
?>
<?php
				  $result10 = mysqli_query($conn , 'select * from search where userId = "'.$_SESSION['uid'].'" ORDER BY id DESC LIMIT 3' );
				  $text = ' ';
				  ...
				  $bKeyword = explode(" ", $text);
				  $query ="SELECT * FROM products WHERE (pName like '%" . $aKeyword[0] . "%'  OR pDesc like '%" . $aKeyword[0] . "%') ";
				  $newArray = array();
				 for($i = 1; $i < count($aKeyword); $i++) {
					if(!empty($aKeyword[$i])) {
						$query .= " AND ( pDesc like '%" . $aKeyword[$i] . "%'";
						$query .= " OR pName like '%" . $aKeyword[$i] . "%')";
					}
				  }
				  //echo count($bKeyword);
				  //print_r($bKeyword);
				  for($i = 0; $i < count($bKeyword); $i++) {
					if(!empty($bKeyword[$i])) {
						$query .= " OR pDesc like '%" . $bKeyword[$i] . "%'";
						$query .= " OR pName like '%" . $bKeyword[$i] . "%'";
					}
				  }
					...
					While($row = $result->fetch_assoc()) 
					{
						...
						for($j = 0; $j < count($aKeyword); $j++) {
							if(!empty($aKeyword[$j])) {
								...
								if (preg_match("/{$aKeyword[$j]}/i", $product)) {
								...
								//echo preg_match("/{$aKeyword[$j]}/i", $product)." ";
							}
						  }
...
<body>
    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content mt-3">
			...
			<?php
				...
				$count = 1;
				 foreach($newArray as $key => $value) {
					   $result = mysqli_query($conn, "select * from products where id='".$key."'");
							...
							<figure class="card card-product">
								<div class="img-wrap " style="padding:10px;">
									<?php
										...
										{
									?>
									<img class="mySlides<?php echo $count; ?>" src="../admin/<?php echo $row1['url']; ?>" style="max-width:100%; max-height:100%; margin:auto;" />
									<?php 
										}
									?>
									<button class="w3-button w3-black w3-display-left" onclick="plusDivs(-1, <?php echo $count-1; ?>)">&#10094;</button>
									<button class="w3-button w3-black w3-display-right" onclick="plusDivs(1, <?php echo $count-1; ?>)">&#10095;</button>
								</div>
								<figcaption class="info-wrap">
										<h4 class="title" style="/*white-space: nowrap; overflow:hidden; text-overflow: ellipsis;*/"><?php echo $row['pName']; ?></h4>
								...
								<div class="bottom-wrap">
									<a href="product.php?p=<?php echo $row['id']; ?>" class="btn btn-sm btn-primary float-right">View Details</a>	
			...
			?>
				<span id="imgNo" data-id="<?php echo $count-1; ?>"></span>
			</div>
        </div> <!-- .content -->
    </div><!-- /#right-panel -->
